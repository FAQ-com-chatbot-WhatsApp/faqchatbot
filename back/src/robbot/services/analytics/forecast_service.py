...
class ForecastService:
    ...
    def __init__(self):
        ...
        if self.has_ml:
            logger.info("✓ ForecastService inicializado com ML capabilities")
        else:
            logger.warning("⚠️ ForecastService inicializado em modo básico (sem ML libs)")
    ...
    async def forecast_demand(
        ...
        try:
            ...
            logger.info(f"✓ Demand forecast gerado para {days_ahead} dias")
            return {
                "status": "success",
                "forecast": forecast,
    ...
    async def predict_lead_conversion_probability(
        ...
        try:
            ...
            logger.info(f"✓ Conversion probability calculada para lead {lead_id}: {total_score:.2f}")
    ...
    async def detect_anomalies(
        ...
        try:
            ...
            if anomalies:
                logger.warning(f"⚠️ {len(anomalies)} anomalias detectadas em {metric_name}")
    ...
    async def recommend_reengagement_time(
        ...
        try:
            ...
            logger.info(f"✓ Reengagement: {day_names[best_day]}s às {best_hour}h")
