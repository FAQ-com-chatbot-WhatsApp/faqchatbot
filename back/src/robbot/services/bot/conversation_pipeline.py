...
class ConversationPipeline:
    ...
    def __init__(
        self, session: Session, transcription_service: Any, vector_store: Any, llm: Any, prompt_templates: Any
    ):
    ...
    async def execute(
        ...
        validation = await self.validator.validate_context(
            user_message=state.message_text, retrieved_context=rag_context, conversation_id=conversation.id
        )
