...
def poll_waha_messages(**_kwargs):
    ...
    try:
        ...
        if messages_processed > 0:
            logger.info(
                "[POLLING] Ciclo concluído. Processadas: %d | Ignoradas: %d", messages_processed, messages_skipped
            )
        elif messages_skipped > 0:
            logger.debug("[POLLING] Ciclo sem novas mensagens. Ignoradas: %d (dedup/filtro)", messages_skipped)
        # Se ambos são 0, nenhuma mensagem foi retornada pela API

        return {"status": "success", "processed": messages_processed, "skipped": messages_skipped}
