...
class WebhookLog(Base):
    ...
    session_name: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    event_type: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="message, message.ack, etc."
    )
    payload: Mapped[dict] = mapped_column(JSONB, nullable=False, comment="Full webhook payload")
