...
class ConversationAnalysisReportSchema(BaseModel):
    ...
    activity_heatmap: list[ActivityHeatmapDataPointSchema] = Field(
        ..., description="Heatmap de atividade por dia/hora"
    )
    top_keywords: list[KeywordFrequencySchema] = Field(
        ..., description="Palavras-chave mais frequentes"
    )
    sentiment_distribution: SentimentDistributionSchema = Field(
        ..., description="Distribuição de sentimentos"
    )
    topic_distribution: list[TopicDistributionSchema] = Field(
        ..., description="Distribuição de topics"
    )
