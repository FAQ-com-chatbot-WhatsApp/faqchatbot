...
def test_create_lead_duplicate_phone_returns_existing(service):
    ...
    # Create first lead
    lead1 = service.create_from_conversation(
        phone_number=phone,
        name="First Name"
    )

    # Try to create duplicate
    lead2 = service.create_from_conversation(
        phone_number=phone,
        name="Second Name"
    )
...
def test_assign_lead_to_user(service):
    ...
    user_id = 123
    updated = service.assign_to_user(
        lead_id=str(lead.id),
        user_id=user_id
    )
...
def test_get_leads_by_status(service):
    ...
    _lead1 = service.create_from_conversation(
        "+5511111222333", "Lead 1"
    )
    _lead2 = service.create_from_conversation(
    ...
    leads = service.get_leads_by_status(
...
def test_get_leads_by_status_with_limit(service):
    ...
    for i in range(5):
        service.create_from_conversation(
            f"+551199988877{i}",
            f"Lead {i}"
        )
...
def test_get_unassigned_leads(service):
    ...
    _lead3 = service.create_from_conversation(
...
def test_list_leads_unassigned_only_filter(service):
    ...
    leads, _total = service.list_leads(
