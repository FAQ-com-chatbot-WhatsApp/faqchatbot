...
def test_get_or_create_new_conversation(conversation_service, db_session):
...
def test_get_or_create_existing_conversation(conversation_service, db_session):
    """Test retrieving an existing conversation."""
    chat_id = "5511888888888@c.us"
    phone_number = "+5511888888888"
    
    # Create first time
    conv1 = conversation_service.get_or_create(
        chat_id=chat_id,
        phone_number=phone_number
    )
    
    # Get same conversation
    conv2 = conversation_service.get_or_create(
        chat_id=chat_id,
        phone_number=phone_number
    )
...
def test_update_status_valid_transition(conversation_service, db_session):
    ...
    updated = conversation_service.update_status(
        ...
        new_status=ConversationStatus.WAITING_SECRETARY
    )
    
    assert updated.status == ConversationStatus.WAITING_SECRETARY


def test_update_status_invalid_transition(conversation_service, db_session):
    ...
    with pytest.raises(BusinessRuleError) as exc_info:
        conversation_service.update_status(
            conversation_id=str(conversation.id),
            new_status=ConversationStatus.TRANSFERRED
        )
...
def test_all_valid_status_transitions(conversation_service, db_session):
    """Test all valid status transitions."""
    conversation = conversation_service.get_or_create(
        chat_id="transitions@c.us",
    ...
    updated = conversation_service.update_status(
        conv_id, ConversationStatus.WAITING_SECRETARY
    )
    assert updated.status == ConversationStatus.WAITING_SECRETARY
    
    # WAITING_SECRETARY -> TRANSFERRED
    updated = conversation_service.update_status(
        conv_id, ConversationStatus.TRANSFERRED
    )
    assert updated.status == ConversationStatus.TRANSFERRED
    
    # TRANSFERRED -> CLOSED
    updated = conversation_service.update_status(
        conv_id, ConversationStatus.CLOSED
    )
...
def test_close_conversation(conversation_service, db_session):
    ...
    closed = conversation_service.close(
        conversation_id=str(conversation.id),
...
def test_transfer_to_secretary(conversation_service, db_session):
    """Test transferring conversation to secretary."""
    conversation = conversation_service.get_or_create(
        chat_id="transfer@c.us",
        phone_number="+5511555555555"
    )
    
    user_id = 123
    transferred = conversation_service.transfer_to_secretary(
        conversation_id=str(conversation.id),
        user_id=user_id
    )
    
    assert transferred.status == ConversationStatus.TRANSFERRED
    assert transferred.assigned_to_user_id == user_id
...
def test_transfer_nonexistent_conversation(conversation_service):
    ...
    with pytest.raises(NotFoundException):
        conversation_service.transfer_to_secretary(
            conversation_id=fake_id,
            user_id=999
        )
...
def test_get_active_conversations(conversation_service, db_session):
...
def test_list_conversations_with_filters(conversation_service, db_session):
...
def test_list_conversations_by_assigned_user(conversation_service, db_session):
    ...
    # Filter by user
    user_convs, total = conversation_service.list_conversations(
        assigned_to_user_id=user_id
    )
    
    assert len(user_convs) == 1
    assert user_convs[0].assigned_to_user_id == user_id
...
def test_get_conversation_by_id(conversation_service, db_session):
    """Test retrieving conversation by ID."""
    created = conversation_service.get_or_create(
        chat_id="getbyid@c.us",
...
def test_update_notes(conversation_service, db_session):
    ...
    updated = conversation_service.update_notes(
        conversation_id=str(conversation.id),
        notes=notes_text
    )
    
    assert updated.notes == notes_text
...
def test_update_notes_conversation_not_found(conversation_service):
    ...
    with pytest.raises(NotFoundException):
        conversation_service.update_notes(
            conversation_id=fake_id,
            notes="Some notes"
        )
...
def test_find_by_criteria_multiple_filters(conversation_service, db_session):
    ...
    assert all(c.assigned_to_user_id == user_id for c in results)
    assert all(c.status == ConversationStatus.TRANSFERRED for c in results)


def test_find_by_criteria_empty_filters(conversation_service, db_session):
