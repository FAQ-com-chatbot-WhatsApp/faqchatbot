...
def test_get_job_status_finished(mock_job_class, queue_service, mock_queue_manager):
    ...
    mock_job.exc_info = None
    mock_job.created_at = datetime(2025, 1, 1, 10, 0, tzinfo=timezone.utc)
    mock_job.started_at = datetime(2025, 1, 1, 10, 1, tzinfo=timezone.utc)
    mock_job.ended_at = datetime(2025, 1, 1, 10, 2, tzinfo=timezone.utc)
...
def test_get_job_status_failed(mock_job_class, queue_service, mock_queue_manager):
    ...
    mock_job.created_at = datetime(2025, 1, 1, tzinfo=timezone.utc)
    mock_job.started_at = datetime(2025, 1, 1, tzinfo=timezone.utc)
    mock_job.ended_at = datetime(2025, 1, 1, tzinfo=timezone.utc)
...
def test_get_failed_jobs(mock_job_class, mock_registry_class, queue_service, mock_queue_manager):
    ...
    mock_job1.ended_at = datetime(2025, 1, 1, tzinfo=timezone.utc)
    mock_job1.exc_info = 'Error: Connection failed'
    
    mock_job2 = MagicMock()
    mock_job2.func_name = 'GeminiAIProcessingJob'
    mock_job2.ended_at = datetime(2025, 1, 2, tzinfo=timezone.utc)
...
def test_enqueue_multiple_jobs_different_queues(queue_service, mock_queue_manager):
    ...
    job1 = queue_service.enqueue_message_processing(
    ...
    job3 = queue_service.enqueue_escalation(
