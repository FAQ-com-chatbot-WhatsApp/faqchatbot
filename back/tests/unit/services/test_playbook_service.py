...
@pytest.fixture()
def playbook_service(db_session, mock_chroma):
...
def test_create_topic(playbook_service, db_session):
...
def test_create_topic_minimal(playbook_service, db_session):
...
def test_get_topic(playbook_service, db_session):
...
def test_list_topics(playbook_service, db_session):
...
def test_update_topic(playbook_service, db_session):
    """Test updating topic fields."""
    topic = playbook_service.create_topic(name="Original Name")
    
    updated = playbook_service.update_topic(
        str(topic.id),
        name="Updated Name",
...
def test_delete_topic(playbook_service, db_session):
...
def test_create_playbook(playbook_service, db_session, mock_chroma):
    ...
    playbook = playbook_service.create_playbook(
        topic_id=str(topic.id),
        name="Botox Information",
        description="Complete botox procedure info",
        active=True
    )
...
def test_create_playbook_indexes_to_chroma(playbook_service, db_session, mock_chroma):
...
def test_get_playbook(playbook_service, db_session):
...
def test_list_playbooks_by_topic(playbook_service, db_session):
...
def test_update_playbook(playbook_service, db_session):
    ...
    updated = playbook_service.update_playbook(
        str(playbook.id),
        name="Updated Name",
        description="New description"
    )
...
def test_delete_playbook(playbook_service, db_session):
...
def test_search_playbooks_no_results(playbook_service, mock_chroma):
    ...
    results = playbook_service.search_playbooks(
        query="botox preço",
        top_k=3
    )
...
def test_full_workflow_create_topic_playbook_steps(playbook_service, db_session):
    ...
    topic = playbook_service.create_topic(
        ...
        description="All aesthetic procedures",
        category="Medical"
    )
    ...
    playbook = playbook_service.create_playbook(
        topic_id=str(topic.id),
        name="Botox Complete Guide",
        description="Complete information about botox"
    )
