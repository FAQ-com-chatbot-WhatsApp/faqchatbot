...
class TestPhase12Security:
    ...
    def test_uc045_to_uc048_mfa_lifecycle(self, api_base_url, dedicated_admin):
        ...
        # --- UC-045: MFA Setup ---
        setup_resp = session.post(f"{api_base_url}/auth/mfa/setup",
                                  json={"password": "TestUser123!Secure"})
        ...
        verify_resp = session.post(f"{api_base_url}/auth/mfa/verify",
                                   json={"code": code})
        ...
        login_resp = login_session.post(f"{api_base_url}/auth/token",
                                        data={"username": email, "password": "TestUser123!Secure"})
        ...
        if login_resp.status_code == 200:
            ...
            if data.get("mfa_required"):
                 ...
                 mfa_login_resp = login_session.post(f"{api_base_url}/auth/mfa/login",
                                                     json={"email": email, "code": totp.now()})
                 assert mfa_login_resp.status_code == 200
                 assert "access_token" in mfa_login_resp.json()

        # --- UC-048: MFA Disable ---
        disable_resp = session.post(f"{api_base_url}/auth/mfa/disable",
                                    json={"password": "TestUser123!Secure", "code": totp.now()})
