...
/*
Esse arquivo eh uma copia fiel do 'administrator\components\com_modules\tmpl\module\edit_assignment.php' apenas com modificacoes das variaveis que eram chamadas com $this e agora são variaveis normais
    * Isso foi necessario para compatibilidade com PHP 8 porque nao podemos mais declarar $this
    * Versao compativel com Joomla 4 e versoes superiores
*/
...
<div class="control-group">
    ...
    <div id="jform_menus" class="controls">
        <select class="form-select" name="jform[assignment]" id="jform_assignment">
            <?php echo HTMLHelper::_('select.options', ModulesHelper::getAssignmentOptions($item->client_id), 'value', 'text', $item->assignment, true); ?>
        </select>
    </div>
</div>
                    ...
                    <li>
                        ...
                        <?php foreach ($type->links as $i => $link) : ?>
                            ...
                            if ($item->assignment == 0) {
                                $selected = 1;
                            } elseif ($item->assignment < 0) {
                                $selected = in_array(-$link->value, $item->assigned);
                            } elseif ($item->assignment > 0) {
                                $selected = in_array($link->value, $item->assigned);
                            }
                            ?>
